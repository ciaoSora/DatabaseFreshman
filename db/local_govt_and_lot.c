/*
Database "local_govt_and_lot" contains tables:
...
#ifndef LOCAL_GOVT_AND_LOT
#define LOCAL_GOVT_AND_LOT

struct T_customer_event_notes {
	double customer_event_note_id;
	double customer_event_id;	// --> customer_events.customer_event_id
	char service_type_code[10];
	double resident_id;
	double property_id;
	char date_moved_in[20];
};

struct T_customer_events {
	double customer_event_id;
	double customer_id;	// --> customers.customer_id
	char date_moved_in[20];	// --> residents.date_moved_in
	double property_id;	// --> residents.property_id
	double resident_id;	// --> residents.resident_id
	double thing_id;	// --> things.thing_id
};

struct T_customers {
	double customer_id;
	char customer_details[23];
};

struct T_organizations {
	double organization_id;
	double parent_organization_id;
	char organization_details[32];
};

struct T_properties {
	double property_id;
	char property_type_code[10];
	char property_address[31];
...
struct T_residents {
	...
	char date_moved_in[20];
	char date_moved_out[20];
	char other_details[22];
};

struct T_residents_services {
	double resident_id;	// --> residents.resident_id
	double service_id;	// --> services.service_id
	char date_moved_in[20];	// --> residents.date_moved_in
	double property_id;	// --> residents.property_id
	char date_requested[20];
	char date_provided[20];
	char other_details[12];
};

struct T_services {
	double service_id;
	double organization_id;	// --> organizations.organization_id
	char service_type_code[11];
	char service_details[12];
};

struct T_things {
	double thing_id;
	double organization_id;	// --> organizations.organization_id
	char type_of_thing_code[12];
	char service_type_code[10];
	char service_details[12];
};

struct T_timed_locations_of_things {
	double thing_id;	// --> things.thing_id
	char date_and_time[20];
	char location_code[16];
};

struct T_timed_status_of_things {
	double thing_id;	// --> things.thing_id
	char date_and_date[20];
	char status_of_thing_code[12];
};
