/*
Database "aircraft" contains tables:
	aircraft
	airport
	airport_aircraft
	match
...
struct T_aircraft {
	...
	char aircraft[22];
	char description[30];
	char max_gross_weight[23];
	char total_disk_area[19];
	char max_disk_loading[23];
};

struct T_airport {
	double airport_id;
	char airport_name[22];
	double total_passengers;
	char %_change_2007[5];
	double international_passengers;
	double domestic_passengers;
	double transit_passengers;
	double aircraft_movements;
	double freight_metric_tonnes;
};

struct T_airport_aircraft {
	double id;
	double airport_id;	// --> airport.airport_id
...
struct T_match {
	...
	char country[21];
	char date[12];
	char fastest_qualifying[14];
	char winning_pilot[2];	// --> pilot.pilot_id
	char winning_aircraft[2];	// --> aircraft.aircraft_id
};
