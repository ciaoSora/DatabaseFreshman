/*
Database "tvshow" contains tables:
	cartoon
	tv_channel
	tv_series
*/
...
struct T_cartoon {
	...
	char directed_by[15];
	char written_by[16];
	char original_air_date[16];
	double production_code;
	char channel[4];	// --> tv_channel.id
};
...
struct T_tv_channel {
	...
	char language[8];
	char content[6];
	char pixel_aspect_ratio_par[11];
	char hight_definition_tv[4];
	char pay_per_view_ppv[3];
	char package_option[22];
};
