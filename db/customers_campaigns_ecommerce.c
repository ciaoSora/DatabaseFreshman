/*
Database "customers_campaigns_ecommerce" contains tables:
	customer_addresses
	customer_orders
	customers
	mailshot_campaigns
...
#ifndef CUSTOMERS_CAMPAIGNS_ECOMMERCE
#define CUSTOMERS_CAMPAIGNS_ECOMMERCE

struct T_customer_addresses {
	double customer_id;	// --> customers.customer_id
	double premise_id;	// --> premises.premise_id
	char date_address_from[20];
	char address_type_code[9];
...
struct T_customer_orders {
	...
	char shipping_method_code[12];
	char order_placed_datetime[20];
	char order_delivered_datetime[20];
	char order_shipping_charges[6];
};

struct T_customers {
	double customer_id;
	char payment_method[13];
	char customer_name[10];
	char customer_phone[20];
	char customer_email[32];
	char customer_address[31];
	char customer_login[5];
	char customer_password[11];
};

struct T_mailshot_campaigns {
	double mailshot_id;
	char product_category[8];
	char mailshot_name[5];
	char mailshot_start_date[20];
	char mailshot_end_date[20];
};

struct T_mailshot_customers {
	double mailshot_id;	// --> mailshot_campaigns.mailshot_id
	double customer_id;	// --> customers.customer_id
	char outcome_code[12];
	char mailshot_customer_date[20];
};

struct T_order_items {
	double item_id;
	char order_item_status_code[10];
	double order_id;	// --> customer_orders.order_id
	double product_id;	// --> products.product_id
	char item_status_code[5];
	char item_delivered_datetime[20];
	char item_order_quantity[2];
};

struct T_premises {
	double premise_id;
	char premises_type[10];
	char premise_details[31];
};

struct T_products {
	double product_id;
	char product_category[12];
...
struct T_customers
customers[] = {
	{  1,  "Credit Card",   "Wendell",   "(773)971-9411x759", "rodriguez.nathanael@example.net",    "618 Webster Stream Apt. 161", "None", "b4459ad261" },
	{  2, "Direct Debit",      "Gage",  "(402)890-2603x6005",          "stark.elna@example.net",              "0260 Walker Trace", "None", "7905d7f1b1" },
	{  3, "Direct Debit",    "Elvera", "1-878-600-7193x8180",  "madyson.mclaughlin@example.org",                "280 Landen Lake", "None", "0b9f651030" },
	{  4, "Direct Debit",    "Stella",    "+43(3)1861877192",          "kristina85@example.org",              "5607 Ryann Canyon", "None", "f64f0fd096" },
	{  5,  "Credit Card", "Magdalena",  "1-352-291-1721x265",     "franecki.danial@example.com",                 "523 Mose Hills", "None", "bdfdc8e91a" },
	{  6, "Direct Debit",  "Carlotta",    "+66(7)1491041398",            "fstanton@example.net",             "31932 Kailey Lodge", "None", "a8467590fd" },
	{  7, "Direct Debit",    "Leanna",  "203-036-0511x96487",       "marc.schiller@example.com", "94105 Bahringer Oval Suite 178", "None", "67e9037197" },
	{  8, "Direct Debit",    "Ruthie", "(840)754-4148x20545",     "elva.mccullough@example.org",   "5514 Ophelia Cliffs Apt. 266", "None", "280160b7b3" },
	{  9,  "Credit Card",    "Donnie",  "(030)521-0681x0191",              "saul30@example.net",  "115 Farrell Squares Suite 599", "None", "22dc8bfaf5" },
	{ 10, "Direct Debit",    "Shanel",      "1-957-676-9414",          "kheathcote@example.org",     "80569 Crona Path Suite 165", "None", "d93857e0c2" },
	{ 11, "Direct Debit",   "Osborne",        "232.171.7032",          "vpowlowski@example.net",             "509 Isabelle Route", "None", "b00e35cc0a" },
	{ 12, "Direct Debit",      "Kyle",        "291-425-7193",      "davis.rosamond@example.net",     "056 Ibrahim Path Suite 669", "None", "341f672c2c" },
	{ 13,  "Credit Card",   "Annabel",        "100.220.3484",  "yoshiko.balistreri@example.org",    "56955 Desmond Lake Apt. 120", "None", "9dc50bc5b6" },
	{ 14, "Direct Debit",  "Geovanny",  "205.496.0690x53058",         "jesus.robel@example.org", "0813 Carolina Bridge Suite 488", "None", "49f32173a2" },
	{ 15, "Direct Debit",  "Mitchell",        "698-327-5792",          "princess75@example.com",       "6735 Daron Lane Apt. 271", "None", "791b1b3e64" }
};
