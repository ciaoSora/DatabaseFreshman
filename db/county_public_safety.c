/*
Database "county_public_safety" contains tables:
	city
	county_public_safety
*/

#ifndef COUNTY_PUBLIC_SAFETY
#define COUNTY_PUBLIC_SAFETY

struct T_city {
	double city_id;
	double county_id;	// --> county_public_safety.county_id
	char name[13];
	double white;
	double black;
	double amerindian;
...
struct T_county_public_safety {
	double county_id;
	char name[16];
	double population;
	double police_officers;
	double residents_per_officer;
	double case_burden;
	double crime_rate;
	char police_force[29];
	char location[6];
};
